...
動作概要
   #. 連携サービスの入力イベント(ポスト/編集など)をトリガーに `parser() <integrations.base.interface.MessageParserInterface.parser>` を呼び出す
   #. `parser() <integrations.base.interface.MessageParserInterface.parser>` は発生イベントの内容を解析し、結果を `MsgData <integrations.protocols.MsgData>` に格納する
   #. `dispatcher.by_keyword() <libs.dispatcher.by_keyword>` が呼び出され、 `MsgData <integrations.protocols.MsgData>` の内容に従い処理を振り分ける
