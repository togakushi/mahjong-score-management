...
| -------------- | ---------------------- | ----------------------------------- | ------------------------------------- | ---------------- | ------------- | ----------------------------------------------------------------- |
| rule_version   | ルール識別子           | 文字列                              | [下記参照](#ルール識別子省略時の挙動) | ←                | ←             |                                                                   |
...
| rank_point     | 順位点                 | カンマ区切り<br>数値4つ(1000点単位) | ---                                   | 30, 10, -10, -30 | 30, 0, -30    | 1位から順に並べる(必要以上の列挙は無視される)                     |
...
| keywords       | 成績記録キーワード     | 文字列<br>(カンマ区切り)            | 空欄                                  | ←                | ←             | `keywords` が空欄のルールセットは参照専用となる                   |
| remarks        | メモ記録用ワード       | 文字列<br>(カンマ区切り)            | 空欄                                  | ←                | ←             | [メモ記録用ワード省略時の挙動](#メモ記録用ワード省略時の挙動)参照 |
...
| remarks | remarks_suffix |                                         登録内容                                          |
| :-----: | :------------: | ----------------------------------------------------------------------------------------- |
|   :x:   |      :x:       | 何も登録されない                                                                          |
|   :o:   |      :x:       | `remarks` の内容が登録される                                                              |
|   :x:   |      :o:       | `keywords` をプレフィックス、 `remarks_suffix` をサフィックスとした組み合わせが登録される |
|   :o:   |      :o:       | `remarks` の内容が登録される                                                              |
