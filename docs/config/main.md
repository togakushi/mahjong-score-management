...
|      キー      |                                    内容                                     |           型           |                  未定義時                  |                                         備考                                          |
| -------------- | --------------------------------------------------------------------------- | ---------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------- |
| rule_config    | ルールセット定義ファイルを指定                                              | 文字列(ファイルパス)   | None                                       | mahjongセクションの定義もない場合、`files/default_rule.ini`が使用される               |
...
|        キー        |        内容        |          型          | 未定義時 |                        備考                        |
| ------------------ | ------------------ | -------------------- | -------- | -------------------------------------------------- |
| <任意のキーワード> | 成績記録キーワード | 文字列(ルール識別子) | 下記参照 | 定義済みの`rule_version`がないマッピングは無視する |

マッピングの設定が行われなかった場合、settingセクションの`keyword`と`default_rule`のマッピングが自動設定される。
