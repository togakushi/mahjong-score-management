...
## 麻雀ルール調整
### mahjongセクション
[settingセクション](#settingセクション)の`rule_config`で定義されたルールセットが存在しない場合に読み込まれるセクション。\
設定可能なパラメータは[ルールセット設定](./ruleset.md)を参照。

オプション扱いのセクションであり、省略可能となる。
...
| default_rule   | ルール識別子未指定に使用するデフォルト値                                    | 文字列                 | デフォルトルールセットの`rule_version` | 未指定時は [mahjongセクション](#mahjongセクション)の`rule_version`を読もうとする      |
...
| remarks_suffix | *成績登録キーワード* と `remarks_suffix` の組み合わせをメモ登録ワードとする | 文字列                 |                                        | カンマ区切りで複数ワードの設定が可能                                                  |
...
> デフォルトルールセット以外を登録したくない場合は、存在しないINIファイルを指定する(無視される)か、空のINIファイルを準備する。
...
|        キー        |        内容        |          型          |                   未定義時                    |                        備考                        |
| ------------------ | ------------------ | -------------------- | --------------------------------------------- | -------------------------------------------------- |
| <任意のキーワード> | 成績記録キーワード | 文字列(ルール識別子) | 値の省略時はmahjongセクションの`rule_version` | 定義済みの`rule_version`がないマッピングは無視する |
...
| -------------------- | ------------------------------------------------------------------------------------- | ------ | -------- | :-----: | :---: | :-----: | :----: | :---: | ---------------------------------------------------- |
| commandword          | 機能を呼び出すキーワード                                                              | 文字列 |          |   〇    |  〇   |   〇    |   〇   |  〇   | カンマ区切りで複数ワードの設定が可能                 |
| command_suffix       | *成績登録キーワード* と `command_suffix` の組み合わせを機能を呼び出すキーワードとする | 文字列 | 空欄     |   〇    |  〇   |   〇    |   〇   |  〇   | カンマ区切りで複数ワードの設定が可能                 |
...
| dropitems            | 非表示にする項目を指定                                                                | 文字列 | None     |   〇    |       |   〇    |        |  〇   | カンマ区切り                                         |
...
| command_suffix     | *成績登録キーワード* と `command_suffix` の組み合わせを機能を呼び出すキーワードとする | 文字列 | 空欄         |                                      |
