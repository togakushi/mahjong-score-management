...
***チャンネル識別子*** は[`settingセクション`](../main.md#settingセクション) などで `channel_id` を定義することで自由に変更できる。\
チャンネル単位でのグループ分けを行ったり、Slackワークスペースの変更対応などで利用できる。

### 集計データ抽出ルール
基本となるデータ抽出条件はルールセット単位となり、「ルール識別子」が抽出条件として指定されている。\
「成績登録ワード」で保存される成績は[マッピング設定](../main.md#keyword_mappingセクション)によってどのルールセットを使うかが定義されている。\
