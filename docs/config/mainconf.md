...
| default_rule  | ルール識別子未指定に使用するデフォルト値       | 文字列                 | デフォルトルールセットの`rule_version` | [mahjongセクション](#mahjongセクション)の`rule_version`                               |
...
| command_suffix       | *成績登録キーワード* と `command_suffix` の組み合わせを機能を呼び出すキーワードとする | 文字列 | 空欄     |   〇    |  〇   |   〇    |   〇   | カンマ区切りで複数ワードの設定が可能                 |
