...
.. graph:: 統計
   ...
   - 指定した対戦数で区切って集計(デフォルト80)

   .. caution::

      :graph:`統計` オプションと同時にプレイヤー/チームの単独指定が必要

.. graph:: 素点
   :category: 統計出力オプション

   「平均順位×平均素点の散布図＋回帰線」を生成する。

   :対戦数: 集計範囲内で集計対象になったゲーム数
   :平均順位: 集計対象ゲームの平均順位
   :平均素点: 集計対象ゲームの平均素点
   :残差: 平均素点と回帰線との差分
...
.. admonition:: 順位素点相関図
   ...
   .. compound::
      ...
      .. image:: regression_ggplot.png
         :scale: 30%
         :alt: 出力サンプル(ggplot)

      .. image:: regression_dark.png
         :scale: 30%
         :alt: 出力サンプル(dark_background)
