...
登録したデータの時間を修正したい
   ...
   .. caution:: 突合処理を動かすと Slack / Discord に記録されている時刻と異なるためデータベースの情報が更新される。
...
Slackのスラッシュコマンドが動かない
   ...
   アプリケーション設定（ :doc:`./setting/for_slack/setup` -> スラッシュコマンド有効化 ）と 設定ファイル（ :ref:`slack-section` ） の ``slash_command`` が一致していない。
