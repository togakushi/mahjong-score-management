...
select
    ...
    count(rank = 1.5 or NULL) as '1.5位',
    round(cast(count(rank = 1.5 or NULL) AS real) / cast(count() as real) * 100, 2) as '1.5位率',
    count(rank = 2 or NULL) as '2位',
    round(cast(count(rank = 2 or NULL) AS real) / cast(count() as real) * 100, 2) as '2位率',
    count(rank = 2.5 or NULL) as '2.5位',
    round(cast(count(rank = 2.5 or NULL) AS real) / cast(count() as real) * 100, 2) as '2.5位率',
    count(rank = 3 or NULL) as '3位',
    round(cast(count(rank = 3 or NULL) AS real) / cast(count() as real) * 100, 2) as '3位率',
    count(rank = 3.5 or NULL) as '3.5位',
